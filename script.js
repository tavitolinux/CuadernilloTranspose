...
// Base de datos de canciones (Generada por Python)
const songDatabase = [
        ...
        { title: 'A7. Abre mis o           jos', url: 'cancion_a7.html' },
        ...
        { title: 'A25. C7add5           Em    C7add5      Em', url: 'cancion_a25.html' },
        ...
        { title: 'B16. Estrofa I:', url: 'cancion_b16.html' },
        ...
        { title: 'C5. D               Cdis Em  A               D', url: 'cancion_c5.html' },
        ...
        { title: 'C22. F.                 C', url: 'cancion_c22.html' },
        ...
        { title: 'C30. C            G/B', url: 'cancion_c30.html' },
        ...
        { title: 'P23. Canto', url: 'cancion_p23.html' },
        ...
        { title: 'U1. Pueblo de Dios toma toda autoridad', url: 'cancion_u1.html' },
...
$(document).ready(function() {
    ...
    function renderSearchResults(filterText) {
        ...
        $('#songSearchResults a').hover(
            function() { $(this).css('background-color', '#f8fafc').css('color', '#2563eb'); },
            function() { $(this).css('background-color', 'transparent').css('color', '#333'); }
        );
    }

    // Inicializar lógica de Index
    if ($('#songList').length > 0) {
        initIndexPage();
    }
    
    // Inicializar lógica de Canción Individual
    if ($('#song-content').length > 0) {
        initSongPage();
    }
});
...
function initIndexPage() {
    ...
    $('#tab-all').click(() => {
        ...
        $('#tab-setlist').removeClass('active');
...
// FORMATO WORD (Márgenes y Line-Height 1 para evitar separación)
window.copySetlistToWord = function() {
    const justTitles = confirm("¿Deseas copiar SÓLO LOS TÍTULOS de las canciones?\n\n- Haz clic en 'Aceptar' para solo los títulos.\n- Haz clic en 'Cancelar' para copiar el repertorio COMPLETO (Ideal para pegar en Word, con formato y espacios continuos).");
        ...
        if (fav && items[i].style.display !== "none") {
            ...
            if (!justTitles) {
                ...
                if (hiddenPre) {
                    ...
                    blockEl.style.whiteSpace = "pre"; // Preserva espacios exactos
                    ...
                    for (let lineData of linesData) {
                        ...
                        // DIVs con interlineado en cero absoluto para Word
    ...
    try {
        ...
        alert("¡Setlist copiado al portapapeles!\n\nAbre Microsoft Word y presiona Pegar. Los espacios e interlineado serán conservados.");
    } catch (err) {
        alert("Tu navegador no soporta el copiado automático.");
    }
    ...
    for (let i = 0; i < items.length; i++) {
        ...
        if (fav && items[i].style.display !== "none") {
            ...
            if (hiddenPre) {
                ...
                // Br simple para saltos en impresión
...
window.sendToWhatsApp = function() {
    if (favorites.length === 0) {
        alert("Tu setlist está vacío. Agrega canciones primero.");
        return;
    }
    alert("⚠️ WhatsApp no permite adjuntar archivos PDF directamente desde páginas web.\n\nA continuación se abrirá la vista de impresión. Por favor:\n\n1. Cambia el Destino de la impresora a 'Guardar como PDF'.\n2. Guarda el archivo en tu dispositivo.\n3. Envíalo manualmente por tu chat de WhatsApp.");
    window.printSetlist();
}
...
function initSongPage() {
    ...
    let origContentEl = document.getElementById('original-content');
    
    originalKeyName = songContentEl.getAttribute('data-key');
    originalText = origContentEl.textContent.trim();
    let currentKey = keysDict.find(k => k.name === originalKeyName) || keysDict.find(k => k.name === 'C');
    let currentFilename = window.location.pathname.split('/').pop() || "";
    
    function renderLocalContent() {
        let pre = document.getElementById('song-content');
        let lines = originalText.split(/\r\n|\n/g);
        let output = [];
        
        let delta = currentKey.value - getKeyByName(originalKeyName).value;
        if (delta < 0) delta += 12;
        ...
        for (let line of lines) {
            ...
            if (isChord) {
                ...
                if (delta !== 0) {
                    transposedLine = transposedLine.replace(chordReplaceRegex, function(match) {
                        let root = getChordRoot(match);
                        let newR = getNewKey(root, delta, currentKey);
                        return newR ? newR.name + match.substr(root.length) : match;
                    });
                }
                output.push(transposedLine.replace(chordReplaceRegex, "<span class='c'>$1</span>"));
            } else {
                output.push(line);
            }
        }
    ...
    $('#transpose-up').click(() => {
        let idx = displayKeys.findIndex(k => k.value === currentKey.value);
        let nextIdx = (idx + 1) % 12;
        currentKey = displayKeys[nextIdx];
        renderLocalContent();
    });

    $('#transpose-down').click(() => {
        let idx = displayKeys.findIndex(k => k.value === currentKey.value);
        let prevIdx = (idx - 1 + 12) % 12;
        currentKey = displayKeys[prevIdx];
        renderLocalContent();
    });

    $('#btn-add-setlist').click(() => {
        let existingIndex = favorites.findIndex(f => f.id === currentFilename);
        let entry = {id: currentFilename, key: currentKey.name};
        ...
        alert(`Añadido al Setlist con éxito (Tono: ${currentKey.name}).`);
    });
}
